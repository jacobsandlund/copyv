...
const file_type_info_map = std.StaticStringMap(FileTypeInfo).initComptime(.{
    ...
    .{ ".l", FileTypeInfo{ .comments = &[_]Comment{.{ .paired = .{ .begin = "/*", .end = "*/" } }}, .indent = .{ .default = .{ .width = 2, .char = ' ' } } } },
...
fn updateChunk(
    ...
    switch (match.comment) {
        ...
        .json => {
            if (std.mem.lastIndexOf(u8, line_remainder, "\"")) |idx| {
                line_remainder = line_remainder[0..idx];
            }
        },
    }
...
fn normalizeWholeFile(bytes: []const u8, comment: Comment) []const u8 {
...
fn getChunkIndent(fc: *FileContext, indent_slice: []const u8) Indent {
    ...
    if (file_indent.enabled) {
        ...
        if (fc.debug_indent != .off) {
            std.log.info("{s}[{d}]: indent start_width={d}", .{
                fc.name,
                fc.line_number,
                start_width.?,
            });
